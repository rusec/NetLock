...
class Beacon {
    ...
    static async scanProcesses() {
        ...
        // Process ended
        let endedProcesses = this.processes.filter((prevProcess) => !currentProcesses.some((currProcess) => currProcess.pid === prevProcess.pid));
        // Process created
        let createdProcesses = currentProcesses.filter((currProcess) => !this.processes.some((prevProcess) => prevProcess.pid === currProcess.pid));
    ...
    static async scanUsers() {
        ...
        let userLoggedOut = this.users.filter((prevUser) => !currentUsers.some((currUser) => currUser.user == prevUser.user &&
            currUser.date == prevUser.date &&
            currUser.time == prevUser.time &&
            currUser.tty == prevUser.tty &&
            currUser.command == currUser.command));
        let usersLoggedIn = currentUsers.filter((currUser) => !this.users.some((prevUser) => currUser.user == prevUser.user &&
            currUser.date == prevUser.date &&
            currUser.time == prevUser.time &&
            currUser.tty == prevUser.tty &&
            currUser.command == currUser.command));
